...
const road=await api(`/api/routes/${rid}/road`,driver.token);
console.log('Real OSRM road:',road.routes[0].distance,'m, waypoint distance',road.waypoints[1].distance);
const browser=await chromium.launch({channel:'msedge',headless:true});
try { const page=await browser.newPage();await page.goto('http://localhost:3000/login');await page.getByLabel('Email address').fill(tag+'@safaitrack.local');await page.getByLabel('Password',{exact:true}).fill(password);await page.getByRole('button',{name:'Sign in',exact:true}).click();await expect(page.getByRole('heading',{name:'Awaiting acceptance'})).toBeVisible();await page.getByRole('button',{name:'Accept',exact:true}).click();await expect(page.locator('.ws-row').filter({hasText:`Route #${rid}`})).toContainText('In progress');
await page.goto(`http://localhost:3000/driver/route/${rid}`);
await expect(page.locator('.collection-truck')).toBeVisible({timeout:25000});
await expect.poll(async()=>Number(await page.locator('.collection-truck').getAttribute('data-traveled-meters'))).toBeGreaterThan(0);
await page.screenshot({path:'test-results/part6-truck-moving.png'});
await expect.poll(async()=>(await api(`/api/routes/${rid}`,driver.token)).status,{timeout:120000,intervals:[1000]}).toBe('Completed');
await expect(page.locator('.collection-marker.collected')).toHaveCount(1);
await expect(page.getByText(/Collected \d/)).toBeVisible();
await page.screenshot({path:'test-results/part6-completed.png'}); }finally{await browser.close();}
const accepted=await api(`/api/routes/${rid}`,driver.token);if(accepted.status!=='Completed')throw Error('Accept failed');
const proof={road:{distance:road.routes[0].distance,coordinates:road.routes[0].geometry.coordinates.length,snappedStopDistance:road.waypoints[1].distance},declined,accepted,routeId:rid,before:'Pending / no driver / no truck',after:assigned,database:sql(`SELECT RouteId,Status,DriverId,TruckId FROM Routes WHERE RouteId=${rid}`)};writeFileSync('test-results/part6-motion.json',JSON.stringify(proof,null,2));console.log(JSON.stringify(proof));
